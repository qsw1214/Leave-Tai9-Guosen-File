<template>
  <div
    class="m-position-select m-dialog-select"
...
<script>
  /**
   * @description 职位选择器，必须放置在form表单内
   * @prop title {Sting} 弹层标题文案
   ...
   * @prop positions {String} sync，传入的话必须sync，当前选中的职位id
   * @prop disabledList {Array} 不能选择的列表会被过滤掉
   ...
   * @example <m-position-select :positions.sync="positions" />
   */

  import position from './position'

  export default {
    name: 'MPositionSelect',
    props: {
      // 参数设置
      title: {
        type: String,
    ...
    data() {
      return {
        storeNameDefault: 'POS_SELECT_LIST',
        storeNameLocal: '',
        search: '',
        choose: [],
        dialogVisible: false,
        ...
        selectPage: 1,
        selectSize: 20,
        positionUrl: this.$api.common.positionList,
        inputIsLoaded: false,
      }
    },
    computed: {
    ...
    created() {
      ...
      if (!this.$store.state[this.storeNameLocal]) {
        this.$store.registerModule(this.storeNameLocal, position)
      }
    ...
    methods: {
      // 处理传入参数
      positionsDeal() {
        this.choose = this.positions.split(',')
        this.chooseList = []
      ...
      positionsHandler() {
        ...
        if (secondSelect) {
          this.$emit('update:positions', this.chooseOutput)
      ...
      optionsHandler() {
        const index = this.options.findIndex(item => this.disabledList.includes(item.id))
      ...
      async fetchData() {
        ...
        const { remote, positionUrl } = this
        const url = remote || positionUrl
      ...
      getTableChange() {
        ...
        const searchKeys = ['code', 'level', 'name', 'pname', 'duty_type_name']

<template>
  <div
    class="m-legal-entity-select m-dialog-select"
...
<script>
  /**
   * @description 成本中心选择器，必须放置在form表单内
   ...
   * @prop legalEntity {String} sync，传入的话必须sync，当前选中的成本中心id
   ...
   * @example <m-legal-entity-select :legal-entity.sync="legalEntity" />
   */

  import legalEntity from './legalEntity'

  export default {
    name: 'MLegalEntitySelect',
    props: {
      // 参数设置
      width: {
        type: Number,
    ...
    data() {
      return {
        storeNameDefault: 'LEGALENTITY_SELECT_LIST',
        storeNameLocal: '',
        search: '',
        choose: [],
        dialogVisible: false,
        ...
        selectPage: 1,
        selectSize: 20,
        orgUrl: this.$api.common.legalEntityList,
        inputIsLoaded: false,
      }
    },
    computed: {
    ...
    created() {
      ...
      if (!this.$store.state[this.storeNameLocal]) {
        this.$store.registerModule(this.storeNameLocal, legalEntity)
      }
    ...
    methods: {
      ...
      // 处理传入参数
      legalEntityDeal() {
        this.choose = this.legalEntity.split(',')
        this.chooseList = []
      ...
      async fetchData() {
        ...
        const { remote, orgUrl, validDate } = this
        const url = remote || orgUrl
        this.$store.commit(`${this.storeNameLocal}/isLoading`, true)
        await this.$axios
          .post(url, { valid_date: this.getDate(validDate) })
          .then((res) => {
            const { data } = res
            // 数据格式化

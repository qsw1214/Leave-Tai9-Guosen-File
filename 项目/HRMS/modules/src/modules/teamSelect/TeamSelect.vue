<template>
  <div
    class="m-team-select m-dialog-select"
...
<script>
  /**
   * @description 团队选择器，必须放置在form表单内
   ...
   * @prop team {String} sync，传入的话必须sync，当前选中的成本中心id
   ...
   * @example <m-team-select :team.sync="team" />
   */
  ...
  export default {
    ...
    data() {
      return {
        storeNameDefault: 'TEAM_SELECT_LIST',
        storeNameLocal: '',
        search: '',
        choose: [],
        dialogVisible: false,
        ...
        selectPage: 1,
        selectSize: 20,
        orgUrl: this.$api.common.teamList,
        inputIsLoaded: false,
      }
    },
    computed: {
    ...
    created() {
      ...
      if (!this.$store.state[this.storeNameLocal]) {
        this.$store.registerModule(this.storeNameLocal, team)
      }
    ...
    methods: {
      ...
      teamHandler() {
        ...
        if (secondSelect) {
          this.$emit('update:team', this.chooseOutput)
